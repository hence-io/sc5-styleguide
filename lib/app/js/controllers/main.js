...
angular.module('sgApp')
  .controller('MainCtrl', function($scope, $location, $state, Styleguide, Variables, localStorageService, Socket) {
    ...
    $scope.markupSection = (Styleguide.config && Styleguide.config.data && !Styleguide.config.data.markupSection) || {isVisible: false};
    $scope.designerTool = (Styleguide.config && Styleguide.config.data && !Styleguide.config.data.designerTool) || {isVisible: false};

    localStorageService.bind($scope, 'markupSection', (Styleguide.config && Styleguide.config.data && !Styleguide.config.data.markupSection) || {isVisible: false});
    localStorageService.bind($scope, 'designerTool', (Styleguide.config && Styleguide.config.data && !Styleguide.config.data.designerTool) || {isVisible: false});
